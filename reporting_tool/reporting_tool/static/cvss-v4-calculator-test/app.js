...
const app = Vue.createApp({
    ...
    methods: {
        ...
        buttonClass(isPrimary, big = false) {
            result = "cvss-btn cvss-btn-m"
            if (isPrimary) {
                result += " cvss-btn-primary"
            }
            if (!big) {
                result += " cvss-btn-sm"
            }
    ...
    computed: {
        vector() {
            ...
            // Instead of use v-model for qualScore, vector and score update the form due the edit function
            console.log("vector value: " + value)
            const inputElement = document.getElementById('id_cvss_vector');
        ...
        macroVector() {
            ...
            macroVector_final = eq1 + eq2 + eq3 + eq4 + eq5 + eq6

            // Instead of use v-model for qualScore, vector and score update the form due the edit function
            const inputElement = document.getElementById('id_macroVector');
                
            if (inputElement) {
                inputElement.value = macroVector_final; 
            }
            return macroVector_final
        },
        score() {
            ...
            // Instead of use v-model for qualScore, vector and score update the form due the edit function
            const inputElement = document.getElementById('id_cvss_score');
        ...
        qualScore() {
            ...
            if (this.score == 0) {
                ...
                qualScore_data =  "Medium"
            }
            else if (this.score < 9.0) {
                qualScore_data =  "High"
            }
            else {
                qualScore_data =  "Critical"
            }

            // Instead of use v-model for qualScore, vector and score update the form due the edit function
            const inputElement = document.getElementById('id_severity');
                
            if (inputElement) {
                inputElement.value = qualScore_data; 
            }

            return qualScore_data
